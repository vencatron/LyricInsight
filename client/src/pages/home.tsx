...
export default function HomePage() {
  ...
  const { mutate, isPending, isError, error, reset } = useMutation({
    mutationFn: async (lyric: string) => {
      ...
      if (!response.ok) {
        throw new Error('Failed to interpret lyric');
      }
  ...
  const handleAlbumSubmit = async (album: string) => {
    ...
    try {
      await fetch('/api/enrich-context', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    <div className="container mx-auto px-4 py-8 max-w-3xl">
      <header className="mb-8 text-center">
        <h1 className="text-3xl md:text-4xl font-bold text-neutral-800 mb-2">
          Kendrick Lamar Lyric Decoder
        </h1>
        <p className="text-neutral-600 md:text-lg">
          Unpack the layers of meaning in Kendrick's thought-provoking lyrics
        </p>
      </header>
      
      <LyricForm onSubmit={handleSubmit} isLoading={isPending} />
      ...
      {!isPending && !isError && results && (
        <ResultsDisplay results={results} onAlbumSubmit={handleAlbumSubmit} />
      )}
      
      {!isPending && !isError && !results && <WelcomeState />}
    </div>
  );
}
